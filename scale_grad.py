...
def sg_loss(logits_perturbed, logits, mask, gamma, eps=1e-8):
    """
    ScaleGrad loss.
    https://github.com/shawnlimn/ScaleGrad

    Args:
        logits_perturbed: reference logit from oracle LM, analogous to target.  [B, T, V]
        logits: current COLD logit under optimization.  [B, T, V]
        mask: mask for "novel" tokens in original implementation; here, for any token
            we want to include in the next steps.  [B, T, V]
    ...
    lprobs = torch.log(new_probs)  # [B, T, V]
    ##########################################################
    target_p = F.softmax(logits_perturbed, dim=-1)
    loss = -(target_p * lprobs).sum(dim=-1).mean(dim=-1)  # [B,]
